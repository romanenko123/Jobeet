<?php
namespace Rim\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
    ...
    {
        $query = $this->em->createQuery('SELECT c FROM RimJobeetBundle:Category c');
        $categories = $query->getResult();

        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM RimJobeetBundle:Job j WHERE j.category = :category AND j.expiresAt > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('RimJobeetBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from RimJobeetBundle:Job j WHERE j.expiresAt > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('RimJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
            ...
            if($jobs_db > 2 ) {
                $jobs_rep = $this->em->getRepository('RimJobeetBundle:Job')->getActiveJobs($category->getId(), 2);
                ...
                $jobs_rep = $this->em->getRepository('RimJobeetBundle:Job')->getActiveJobs($category->getId(), 2, 1);
    ...
    {
        $query = $this->em->createQuery('SELECT j FROM RimJobeetBundle:Job j WHERE j.expiresAt > :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_db = $query->getSingleResult();

        $job_rep = $this->em->getRepository('RimJobeetBundle:Job')->getActiveJob($job_db->getId());
        // If the job is active, the getActiveJob() method should return a non-null value
        $this->assertNotNull($job_rep);

        $query = $this->em->createQuery('SELECT j FROM RimJobeetBundle:Job j WHERE j.expiresAt < :date');         $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_expired = $query->getSingleResult();

        $job_rep = $this->em->getRepository('RimJobeetBundle:Job')->getActiveJob($job_expired->getId());
